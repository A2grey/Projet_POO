...
if argv[0] == "communaute.py":
    ...
    if nb_arguments > 3:
        if argv[1] == 'influence_par':
            if __name__ == "__main__":
                from auteur import influence_par
                auteur = ""
                for i in range(2, nb_arguments-1):
                    auteur += " " + argv[i]
                influence_par(auteur.strip(),int(argv[nb_arguments-1]))

    if nb_arguments > 3:
        if argv[1] == 'influence':
            if __name__ == "__main__":
                from auteur import influence
                influence(argv[2],int(argv[nb_arguments-1]))


    if nb_arguments > 3:
        if argv[1] == 'communautes':
            if __name__ == "__main__":
                from auteur import communautes
                communautes(argv[2],int(argv[nb_arguments-1]))
                ...
                #for i in range(2, nb_arguments-1):
                    #auteur += " " + argv[i]
                #communautes(auteur.strip().upper(),int(argv[nb_arguments-1]))
